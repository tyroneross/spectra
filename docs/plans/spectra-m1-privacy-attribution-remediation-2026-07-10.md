...
| Release helper inventory | macos/Makefile + build-and-refresh | Exact required helpers present, architecture-compatible, hardened, and signed in the requested mode or release fails | daemon-core and text-render omitted; missing helpers skipped; host-only helpers can mismatch the universal app; nested failures can be swallowed | Add both, prebuild universal helpers, strict shell failure propagation, architecture parity, and mode-aware signature validation | Makefile/build script | built bundle inventory + lipo + codesign |
...
| Release packaging requires complete architecture-compatible helper inventory | signed app silently ships incomplete or helpers fail on Intel | strict `embed-helpers`, lipo parity, and signed bundle inventory check |
