...
- **Remaining backlog** (smaller / larger-scoped, none blocking): make `handleCapture` RETURN the ScreenshotResult union so tsc enforces the branch shapes (shared with walkthrough — a real refactor; the `as` cast at core-impl.ts:322 currently bypasses the check); daemon server-side param validation; screenshot intelligence-mode (element/region/auto) corpus coverage; vestigial `error` field on StartRecordingResult; M3 external-daemon seeding impl. Registered `.build-loop/backlog` feature items (CDP script-runner [overlaps Codex M4], audio, voiceover, dist-sync) are separate product work.
