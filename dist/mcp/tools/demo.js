...
import { recordComposite } from '../../media/composite-recorder.js';
// ─── Zod schema ───────────────────────────────────────────────
...
// ─── Handler ──────────────────────────────────────────────────
export async function handleDemo(params, ctx) {
    ...
    if (parsed.action === 'record-composite') {
        ...
        if (result.ok && result.output && parsed.sessionId && ctx) {
            try {
                ...
                result.warnings.push(`Could not register artifact against session ${parsed.sessionId} (no active run).`);
            }
        }
