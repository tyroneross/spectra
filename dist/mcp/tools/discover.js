...
// ─── Handler ─────────────────────────────────────────────────
export async function handleDiscover(params, ctx) {
    ...
    const viewport = { width: 1280, height: 800, devicePixelRatio: 1 };
    if (DEBUG) {
    ...
    const snapshotCache = graph._snapshotCache;
    ...
    const serializable = {
        ...
        navigation: manifest.navigation ? {
            nodes: Object.fromEntries(Array.from(manifest.navigation.nodes).map(([k, v]) => [k, { ...v, screenshot: undefined }])),
