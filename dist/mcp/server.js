...
const MCP_IDLE_TIMEOUT_MS = (() => {
    ...
    if (raw === undefined || raw === '')
        return 0; // disabled unless opted in
    const n = Number(raw);
...
function installIdleTimeout(transport) {
    if (MCP_IDLE_TIMEOUT_MS === 0)
        return;
    let lastFrameAt = Date.now();
    const pollMs = Math.min(MCP_IDLE_TIMEOUT_MS, 5_000);
    const timer = setInterval(() => {
        if (Date.now() - lastFrameAt >= MCP_IDLE_TIMEOUT_MS)
            lifecycleExit();
    }, pollMs);
    timer.unref();
