...
export function buildZoomTrack(clicks, totalMs, fps, opts = {}) {
    ...
    const segments = mergeSegments(buildSegments(sourceClicks, totalMs, scale, opts), opts.mergeGapMs ?? DEFAULT_MERGE_GAP_MS);
    const track = [];
    let segmentIndex = 0;
...
export function buildTimedZoomTrack(windows, totalMs, fps, opts = {}) {
    if (!Number.isFinite(totalMs) || totalMs < 0) {
        throw new Error('totalMs must be a non-negative finite number');
    }
    if (!Number.isFinite(fps) || fps <= 0) {
...
function buildSegments(clicks, totalMs, scale, opts) {
    ...
    return clicks
        .filter((click) => Number.isFinite(click.tMs)
...
function normalizeTimedWindows(windows, totalMs) {
    return windows
...
function scaleAt(tMs, segment, targetScale, opts) {
...
function timedScaleAt(tMs, window, opts) {
...
function centerAt(tMs, segment, scale) {
    ...
    for (let index = 0; index < clicks.length - 1; index += 1) {
        ...
        if (tMs <= next.tMs) {
            ...
            return clampCenter(current.cx + (next.cx - current.cx) * p, current.cy + (next.cy - current.cy) * p, scale);
        }
    }
    const last = clicks[clicks.length - 1];
    return clampCenter(last.cx, last.cy, scale);
}
function deriveDwellClicks(cursorPath, totalMs, opts) {
...
//# sourceMappingURL=zoom-keyframes.js.map