...
const SEARCH_SELECTOR = 'input[type="search"], input[type="text"], [role="searchbox"], [role="search"] input, input:not([type=hidden])';
function sleep(ms) {
...
async function locateTarget(runtime, target) {
    const expr = `(function(){
    ...
    const result = (await runtime.evaluate(expr));
    return result ?? null;
}
async function runSearch(ctx, value) {
    ...
    if (!located)
        return { ok: false, detail: 'search input not found' };
...
async function runClick(ctx, target) {
    const hit = await locateTarget(ctx.runtime, target);
    if (!hit)
        return { ok: false, detail: `target "${target}" not found` };
    await ctx.input.click(hit.x, hit.y);
    return { ok: true, detail: `clicked "${hit.label}" at (${hit.x},${hit.y})` };
}
async function runScroll(ctx) {
    // Real mouse-wheel scroll near the viewport center.
    const size = (await ctx.runtime.evaluate(`({ w: window.innerWidth, h: window.innerHeight, before: window.scrollY })`));
    ...
    await sleep(60);
    let after = (await ctx.runtime.evaluate('window.scrollY'));
    if (!(after > (size.before ?? 0))) {
        ...
        await sleep(60);
        after = (await ctx.runtime.evaluate('window.scrollY'));
 ...
 */
export async function runDemoScript(script, opts) {
