...
function scoreElements(elements, intentLower) {
    ...
    for (const el of elements) {
        ...
        if (labelRegex.test(intentLower)) {
            ...
            const intentWords = intentLower.split(/\s+/);
            const matchedWords = intentWords.filter((w) => w.length > 2 && labelLower.includes(w));
...
function resolveAlgorithmic(options) {
    ...
    if (hints.ordinal !== undefined) {
        ...
        for (let i = 0; i < words.length; i++) {
            if ([...roles].some((role) => scoreRole(words[i], role) > 0))
                roleIndex = i;
        }
        ...
        if (roleIndex >= 0 && ordinalIndex >= 0) {
            ...
            const matches = elements.filter((el) => scoreRole(roleWord, el.role) > 0
...
function scoreRole(intent, role) {
    ...
    for (const word of intentWords) {
        if (word === roleLower)
            return 1.0;
        // Common synonyms
        if (word === 'btn' && roleLower === 'button')
            return 0.8;
        if (word === 'input' && roleLower === 'textfield')
            return 0.8;
        if (word === 'text' && roleLower === 'textfield')
            return 0.6;
    }
    return 0;
}
function scoreLabelSimilarity(intent, label) {
    ...
    if (intent.includes(labelLower))
        return 1.0;
    if (labelLower.includes(intent.trim()))
        return 0.9;
...
export function parseSpatialHints(intent) {
    const hints = {};
    // ── Position (legacy compat) ──
    if (/\bfirst\b/.test(intent))
        hints.position = 'first';
    else if (/\blast\b/.test(intent))
        hints.position = 'last';
    else if (/\btop\b/.test(intent))
        hints.position = 'top';
    else if (/\bbottom\b/.test(intent))
        hints.position = 'bottom';
    const nearMatch = intent.match(/\b(?:next to|near|beside|by)\s+(.+?)(?:\s*$)/);
    if (nearMatch)
        hints.near = nearMatch[1].trim();
    // ── Ordinal (Phase 4) ──
    const ordinalMatch = intent.match(/\b(first|second|third|fourth|fifth|1st|2nd|3rd|4th|5th)\b/i);
    if (ordinalMatch) {
        const ordinals = {
    ...
    const dirMatch = intent.match(/\b(above|below|under|left of|right of|near)\s+(?:the\s+)?(.+?)(?:\s*$)/i);
    if (dirMatch) {
        const dirMap = {
...
function scoreSpatial(hints, el, index, allElements) {
    if (!hints.position && !hints.near)
        return 0;
    let score = 0;
    if (hints.position) {
