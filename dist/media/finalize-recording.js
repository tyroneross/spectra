 ...
 */
export function buildFinalizeArgs(input, output, action, hasAudio) {
    if (action === 'remux') {
...
export function buildFinalizeProbeArgs(path) {
    return [
        '-v', 'error',
 ...
 */
export async function probeForFinalize(path) {
    ...
    const durationSeconds = numberFromString(video?.duration) ?? numberFromString(data.format?.duration);
    return {
 ...
 */
export async function hasMoovAtom(path) {
...
export async function finalizeRecording(params) {
    ...
    if (rawStat.size < MIN_VALID_BYTES) {
        throw new RecordingFinalizeError(`Raw recording at ${rawPath} is ${rawStat.size} bytes (< ${MIN_VALID_BYTES}); `
            + 'the native recorder produced a stub with no usable video.');
    }
    if (!detectFfmpeg()) {
        throw new RecordingFinalizeError('ffmpeg not found — cannot finalize recording. Install with: brew install ffmpeg');
    }
    const sourceProbe = await probeForFinalize(rawPath);
    if (!sourceProbe || !sourceProbe.hasVideoStream) {
        throw new RecordingFinalizeError(`Raw recording at ${rawPath} has no decodable video stream.`);
    }
    if (!sourceProbe.durationMs || sourceProbe.durationMs <= 0) {
        throw new RecordingFinalizeError(`Raw recording at ${rawPath} has zero duration.`);
    }
    const action = sourceProbe.codec === 'h264' && sourceProbe.pixFmt === 'yuv420p' ? 'remux' : 'transcode';
    ...
    if (exitCode !== 0) {
        ...
        throw new RecordingFinalizeError(`ffmpeg ${action} failed (exit ${exitCode})${detail ? `: ${detail.trim()}` : ''}`);
    }
    ...
    if (!outStat || outStat.size < MIN_VALID_BYTES) {
        throw new RecordingFinalizeError(`Finalized output ${outPath} is ${outStat?.size ?? 'missing'} bytes (< ${MIN_VALID_BYTES}).`);
    }
    if (!(await hasMoovAtom(outPath))) {
        throw new RecordingFinalizeError(`Finalized output ${outPath} has no moov atom — not a playable mp4.`);
    }
    const outProbe = await probeForFinalize(outPath);
    if (!outProbe || !outProbe.hasVideoStream) {
        throw new RecordingFinalizeError(`Finalized output ${outPath} has no video stream after ${action}.`);
    }
    if (!outProbe.durationMs || outProbe.durationMs <= 0) {
        throw new RecordingFinalizeError(`Finalized output ${outPath} has zero duration after ${action}.`);
    }
    if (outProbe.pixFmt !== 'yuv420p') {
        throw new RecordingFinalizeError(`Finalized output ${outPath} is pix_fmt=${outProbe.pixFmt ?? 'unknown'}, expected yuv420p.`);
    }
    return {
        path: outPath,
        action,
...
//# sourceMappingURL=finalize-recording.js.map