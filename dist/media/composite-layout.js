 ...
 */
export function computeSplitLayout(displayWidth, displayHeight, override) {
    if (override)
        return override;
    if (!Number.isFinite(displayWidth) || !Number.isFinite(displayHeight)) {
        throw new Error(`Invalid display dimensions for split layout: ${displayWidth}×${displayHeight} (expected finite numbers)`);
    }
    const width = Math.floor(displayWidth);
    const height = Math.floor(displayHeight);
    if (width < 2 || height < 1) {
        throw new Error(`Invalid display dimensions for split layout: ${width}×${height} (need width ≥ 2, height ≥ 1)`);
    }
...
//# sourceMappingURL=composite-layout.js.map