...
function parseBuildSettings(output) {
    ...
    for (const line of output.split('\n')) {
        ...
        if (m) {
            const k = m[1];
            const v = m[2].trim();
            if (k === 'BUILT_PRODUCTS_DIR')
                out.BUILT_PRODUCTS_DIR = v;
            else if (k === 'EXECUTABLE_NAME')
                out.EXECUTABLE_NAME = v;
            else if (k === 'WRAPPER_NAME')
                out.WRAPPER_NAME = v;
...
async function runXcodebuild(xcodeTarget, spawnImpl, timeoutMs) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const out = [];
        const err = [];
        proc.stdout?.on('data', (c) => out.push(c.toString('utf8')));
        proc.stderr?.on('data', (c) => err.push(c.toString('utf8')));
        const timer = setTimeout(() => {
            try {
                proc.kill();
            }
            catch { /* best-effort */ }
            reject(new LauncherError(`xcodebuild -showBuildSettings timed out after ${timeoutMs}ms`, err.join('').slice(-400)));
        }, timeoutMs);
        proc.on('error', (e) => {
            clearTimeout(timer);
            reject(new LauncherError(`xcodebuild not found or failed to spawn: ${e.message}`, 'Install Xcode command-line tools: xcode-select --install'));
        });
        proc.on('exit', (code) => {
            clearTimeout(timer);
            if (code === 0) {
                ...
                reject(new LauncherError(`xcodebuild exited with code ${code}`, err.join('').slice(-400)));
            }
        });
    });
}
export async function launchMacosApp(opts) {
    if (!opts.detection.xcodeTarget) {
        throw new LauncherError('detection.xcodeTarget is required for macOS launch', 'Detection should have populated xcodeTarget.');
    }
    ...
    if (!settings.BUILT_PRODUCTS_DIR || !wrapper) {
        throw new LauncherError('xcodebuild did not report BUILT_PRODUCTS_DIR or a product wrapper name', 'The Xcode project may not have a buildable macOS app target.');
    }
    const appPath = `${settings.BUILT_PRODUCTS_DIR}/${wrapper}`;
    if (!existsSync(appPath) && !opts.dryRun) {
        throw new LauncherError(`Built .app does not exist at ${appPath}`, 'Run an Xcode build first, or open the project in Xcode to build it.');
    }
    ...
    const proc = spawnImpl('open', [appPath], { stdio: 'ignore' });
    await new Promise((resolve, reject) => {
        proc.on('error', (e) => reject(new LauncherError(`Failed to invoke /usr/bin/open: ${e.message}`, 'Check permissions on the .app bundle.')));
        proc.on('exit', (code) => {
            if (code === 0)
                resolve();
            else
                reject(new LauncherError(`open exited with code ${code}`, undefined));
        });
    });
    return {
        kind: 'macos',
