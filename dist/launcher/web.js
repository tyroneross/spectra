...
export async function launchWebDevServer(opts) {
    if (!opts.detection.startCommand || opts.detection.startCommand.length === 0) {
        throw new LauncherError(`No startCommand for kind ${opts.detection.kind}`, 'Detection should have populated startCommand.');
    }
    ...
    const url = await new Promise((resolve, reject) => {
        ...
        proc.on('error', (err) => {
            ...
            resolved = true;
            clearTimeout(timer);
            reject(new LauncherError(`Failed to spawn dev server: ${err.message}`, 'Check that the package manager (npm/npx) is installed.'));
        });
        proc.on('exit', (code, signal) => {
            ...
            resolved = true;
            clearTimeout(timer);
            reject(new LauncherError(`Dev server exited before binding a URL (code=${code} signal=${signal})`, `Recent output: ${buffers.join('').slice(-500)}`));
        });
        const timer = setTimeout(() => {
            ...
            reject(new LauncherError(`Dev server did not print a URL within ${timeoutMs}ms`, `Recent output: ${buffers.join('').slice(-500)}`));
        }, timeoutMs);
    });
    return {
