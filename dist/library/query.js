export function find(all, opts) {
    ...
    if (opts.feature)
        out = out.filter((c) => c.feature === opts.feature);
    if (opts.component)
        out = out.filter((c) => c.component === opts.component);
    if (opts.platform)
        out = out.filter((c) => c.platform === opts.platform);
    if (opts.type)
        out = out.filter((c) => c.type === opts.type);
    if (opts.starred === true)
        out = out.filter((c) => c.starred === true);
    if (opts.since) {
        const t = Date.parse(opts.since);
        if (!Number.isNaN(t))
            out = out.filter((c) => Date.parse(c.created_at) >= t);
    }
    if (opts.until) {
...
export function groupBy(all, by) {
    ...
    for (const c of all) {
        ...
        if (by === 'feature')
            ...
            key = c.component || '(none)';
        else if (by === 'platform')
            key = c.platform;
        else if (by === 'type')
            key = c.type;
        else if (by === 'date')
            key = c.created_at.slice(0, 10);
...
export function stats(all) {
    ...
    for (const c of all) {
        ...
        if (c.feature)
            s.by_feature[c.feature] = (s.by_feature[c.feature] ?? 0) + 1;
        s.total_size_bytes += c.size_bytes || 0;
        if (c.starred)
            s.starred_count += 1;
    }
    if (all.length) {
        const sorted = all.slice().sort((a, b) => Date.parse(a.created_at) - Date.parse(b.created_at));
