 ...
 */
export async function exportCaptures(captures, opts) {
...
function renderManifest(captures, opts) {
    const lines = [];
    lines.push('# Spectra Library Export');
    lines.push('');
    lines.push(`Exported ${captures.length} capture${captures.length === 1 ? '' : 's'} from the spectra library. Each entry below names the media file (relative to this manifest) plus the recorded metadata.`);
    lines.push('');
    for (const c of captures) {
        ...
        if (c.device_name)
            lines.push(`- **Device**: ${c.device_name}`);
        if (c.duration_ms)
            lines.push(`- **Duration**: ${(c.duration_ms / 1000).toFixed(1)}s`);
        lines.push(`- **Captured**: ${c.created_at}`);
        if (c.git_branch || c.git_commit) {
            lines.push(`- **Git**: ${c.git_branch ?? '?'}${c.git_commit ? `@${c.git_commit.slice(0, 7)}` : ''}`);
        }
        if (c.walkthrough) {
