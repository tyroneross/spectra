...
function nonStructural(elements) {
    return elements.filter(el => !STRUCTURAL_ROLES.has(el.role));
}
export function detectState(snapshot) {
    const { elements } = snapshot;
    const nonStruct = nonStructural(elements);
    const loading = { score: 0, ids: [] };
    const error = { score: 0, ids: [] };
    ...
    if (hasHeading && hasContent) {
        ...
        if (heading)
            populated.ids.push(heading.id);
        if (content)
            populated.ids.push(content.id);
    }
    const hasLoadingOrErrorOrEmpty = loading.score > 0 || error.score > 0 || empty.score > 0;
    ...
    if (focusedInteractiveId !== null && winner.state === 'populated') {
        const confidence = winner.accum.score / (winner.accum.score + runnerUp.accum.score + 1);
...
async function evaluate(conn, sessionId, expression) {
    if (!conn)
        return;
    try {
...
export function createStateTriggers(optionsOrDriver, legacyPlatform) {
    // Legacy two-arg call: (driver, platform) — no CDP connection available
    if (legacyPlatform !== undefined) {
        return [];
    }
    const options = optionsOrDriver;
