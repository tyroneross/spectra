...
function scorePosition(el, viewport) {
    const [x, y] = el.bounds;
...
function scoreInteractivity(el) {
...
function scoreLabelQuality(el) {
    ...
    if (len === 0) {
        ...
        value = 0.2;
        reason = 'single-char label';
    }
    else if (len <= 20) {
        value = 1.0;
        reason = `short label (${len} chars)`;
    }
    else if (len <= 50) {
        value = 1.0;
        reason = `medium label (${len} chars)`;
    }
    else {
        value = 0.5;
        reason = `long label (${len} chars)`;
    }
    return {
        name: 'label_quality',
...
function scoreContentDensity(el, all) {
...
function scoreVisualProminence(el, viewport) {
    const [, , w, h] = el.bounds;
...
// ─── Public API ───────────────────────────────────────────────────────────────
export function scoreElements(elements, viewport) {
    ...
    const scores = elements.map(el => {
        ...
        const score = clamp(factors.reduce((sum, f) => sum + f.weight * f.value, 0));
        return { elementId: el.id, score, factors };
    });
    return scores.sort((a, b) => b.score - a.score);
}
// ─── Region detection ─────────────────────────────────────────────────────────
export function findRegions(scores, elements) {
    ...
    const highEls = [...highIds].map(id => elemMap.get(id)).filter((e) => !!e);
    if (highEls.length === 0)
        return [];
    // Union-Find
