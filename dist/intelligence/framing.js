...
import { regionLabel, boundingBox, clusterElements } from './spatial.js';
...
function applyPadding(x, y, w, h, padding, imgW, imgH) {
 ...
 * caused ratio drift.
 */
function applyAspectRatio(x, y, w, h, targetRatio, imgW, imgH) {
...
// ─── Public API ───────────────────────────────────────────────────────────────
export function frame(screenshot, scores, elements, options) {
    ...
    if (target === 'viewport' || target === 'fullpage') {
        ...
        if (region) {
            ...
            labelElements = region.elements.map(id => elemMap.get(id)).filter((e) => !!e);
        }
    }
    else {
        ...
        if (qualifying.length === 0) {
            ...
            qualifying = top25.map(s => elemMap.get(s.elementId)).filter((e) => !!e);
        }
        if (qualifying.length === 0) {
    ...
    if (options?.aspectRatio !== undefined && options.aspectRatio > 0) {
        [cropX, cropY, cropW, cropH] = applyAspectRatio(cropX, cropY, cropW, cropH, options.aspectRatio, imgW, imgH);
    }
...
export function autoFrame(screenshot, scores, elements) {
