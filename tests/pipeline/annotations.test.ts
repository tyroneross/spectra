...
describe('timed step cards', () => {
  ...
  it('builds an ffmpeg card overlay graph', () => {
    ...
    expect(filter).toContain("y='158+24*max(0\\,1-(t-0)/0.3)'")
    expect(filter).toContain("y='158+24*max(0\\,1-(t-1.2)/0.3)'")
  ...
  pilIt('uses rendered PNG overlays when Pillow is available', async () => {
    ...
    expect(plan.filter).toContain("y='0+24*max(0\\,1-(t-0)/0.3)'")
    expect(plan.filter).toContain("y='0+24*max(0\\,1-(t-1.2)/0.3)'")
