 ...
 * surfaces MUST agree.
 */
export function slugFor(binaryPathOrName: string): string {
...
export function identifierFor(binaryPathOrName: string): string {
...
let cachedDeveloperId: string | null | undefined
...
export function markDevidUnavailable(): void {
 ...
 */
export function detectDeveloperIdIdentity(): string | null {
  if (cachedDeveloperId !== undefined) return cachedDeveloperId
 ...
 */
export function resolveSigningIdentity(): ResolvedSigningIdentity {
  if (process.env.SPECTRA_CODESIGN === '0') return { identity: null, mode: 'skip' }

  const explicit = process.env.SPECTRA_CODESIGN_IDENTITY?.trim()
  if (explicit) {
    if (explicit === 'skip') return { identity: null, mode: 'skip' }
    if (explicit === ADHOC_IDENTITY) return { identity: ADHOC_IDENTITY, mode: 'adhoc' }
  ...
  if (process.env.SPECTRA_STABLE_SIGNING !== '0' && !devidUnavailable) {
    ...
    if (devId) return { identity: devId, mode: 'devid' }
  }
...
export function readSignatureInfo(binaryPath: string): SignatureInfo {
  const empty: SignatureInfo = { identifier: null, cdhash: null, teamId: null, adhoc: false }
  if (!existsSync(binaryPath)) return empty
  try {
...
export function readCdhash(binaryPath: string): string | null {
  return readSignatureInfo(binaryPath).cdhash
}

// ─── Signing manifest: what each helper was signed as, at install time ───────
...
export function readSigningManifest(): SigningManifest {
  return readJsonFile<SigningManifest>(SIGNING_MANIFEST_PATH, {})
}

export function readSigningManifestEntry(binaryPathOrName: string): SigningManifestEntry | undefined {
 ...
 * into a build.
 */
export function recordSigningManifest(binaryPath: string): SigningManifestEntry {
  const info = readSignatureInfo(binaryPath)
  const entry: SigningManifestEntry = {
...
// ─── Permission-grant tracking: cdhash a permission was last granted against ─
...
export function readPermissionGrants(): PermissionGrants {
  return readJsonFile<PermissionGrants>(PERMISSION_GRANTS_PATH, {})
}
 ...
 * be read.
 */
export function recordGrant(permission: string, helperBinary: string): void {
...
export type StalenessVerdict =
  | { stale: false }
  | { stale: true; grantedCdhash: string; currentCdhash: string | null }
 ...
 */
export function assessGrantStaleness(permission: string, helperBinary: string): StalenessVerdict {
  const grant = readPermissionGrants()[permission]
  if (!grant) return { stale: false }
  const currentCdhash = readCdhash(helperBinary)
  if (currentCdhash && currentCdhash === grant.grantedCdhash) return { stale: false }
  return { stale: true, grantedCdhash: grant.grantedCdhash, currentCdhash }
}

// ─── Install-flow guard: warn when a helper's signing identity changed ───────

// Written by scripts/codesign-native.sh's install guard (flat, hand-built JSON).
...
export function readRegrantMarker(): RegrantMarker | null {
  return readJsonFile<RegrantMarker | null>(REGRANT_MARKER_PATH, null)
}

export function clearRegrantMarker(): void {
  try {
    if (existsSync(REGRANT_MARKER_PATH)) rmSync(REGRANT_MARKER_PATH, { force: true })
  } catch {
    // Non-fatal.
  }
}
...
export {
  ...
  PERMISSION_GRANTS_PATH,
  REGRANT_MARKER_PATH,
  BIN_DIR as SIGNING_BIN_DIR,
}
