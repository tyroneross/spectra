...
const FORM_SUBMIT_PATTERN =
  /\b(submit|save|send|sign\s?in|log\s?in|login|register|sign\s?up|checkout|pay|purchase|confirm)\b/i
const DESTRUCTIVE_PATTERN =
  /\b(delete|remove|destroy|archive|reset|clear|discard|sign\s?out|log\s?out|logout|deactivate)\b/i

export function extractActionValue(intent: string): string | undefined {
...
export function inferActionFromIntent(intent: string): ActionType {
  const lower = intent.toLowerCase()
  if (/\b(clear|empty|erase)\b/.test(lower)) return 'clear'
  if (/\b(type|enter|fill|write|input)\b/.test(lower)) return 'type'
  if (/\b(scroll|swipe)\b/.test(lower)) return 'scroll'
  if (/\b(hover|mouse over)\b/.test(lower)) return 'hover'
  if (/\b(focus)\b/.test(lower)) return 'focus'
  if (/\b(select|choose|pick)\b/.test(lower)) return 'select'
  return 'click'
}

export function isPotentiallyUnsafeForNavigation(element: Element): boolean {
...
export function isElementActionable(element: Element): boolean {
  ...
  if (element.actions.length === 0 && !CLICK_ROLES.has(normalizeRole(element.role))) return false
  return true
}

export function selectActionForElement(
  element: Element,
  options: ActionSelectionOptions = {},
): ActionSelection | null {
  if (!isElementActionable(element)) return null
...
function selectNavigationAction(element: Element): ActionSelection | null {
...
function supportsAction(element: Element, action: ActionType): boolean {
  const role = normalizeRole(element.role)
