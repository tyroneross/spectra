...
import type { AxBridgePort, RawVisionGrounding } from './port.js'
...
function roleForVisionLabel(label: string): string {
  if (FIELD_LABEL_HINT.test(label) || /:\s*$/.test(label)) return 'AXTextField'
  if (BUTTON_LABEL_HINT.test(label)) return 'AXButton'
  return 'AXStaticText'
}

function actionsForVisionRole(role: string): string[] {
  if (role === 'AXTextField') return ['press', 'setValue']
  return ['press']
}

function normalizeGrounding(raw: RawVisionGrounding): AxNode | undefined {
...
export class NativeVisionFallback implements VisionFallback {
  readonly name = 'native-vision-fallback'
  readonly unavailableReason?: string
  private readonly usable: boolean
  private readonly target?: AxTarget

  constructor(
    private readonly port: AxBridgePort,
    options: NativeVisionFallbackOptions,
  ) {
  ...
  static async create(port: AxBridgePort, target?: AxTarget): Promise<NativeVisionFallback> {
  ...
  async ground(target: AxTarget | undefined, _context?: VisionContext): Promise<AxNode[]> {
    if (!this.usable) {
    ...
    return grounded
      ...
      .filter((node): node is AxNode => node !== undefined)
  }
}
