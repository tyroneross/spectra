...
server.tool(
  ...
  async ({ sessionId, type, preset, mode, elementId, region, aspectRatio, clean, quality, fps, codec, bitrate, hardware, composite }) => {
    return wrapHandler(
      () => handleCapture({ sessionId, type, preset, mode, elementId, region, aspectRatio, clean, quality, fps, codec, bitrate, hardware, composite }, ctx),
      'spectra_capture',
    )
  },
)
...
server.tool(
  ...
  { readOnlyHint: false, destructiveHint: true, idempotentHint: false },
  async ({ sessionId, actions, continueOnError }) => {
    return wrapHandler(
      () => handleLlmStep({ sessionId, actions, continueOnError }, ctx),
      'spectra_llm_step',
    )
  },
)
