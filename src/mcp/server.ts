...
type TextOrImage = { type: 'text'; text: string } | { type: 'image'; data: string; mimeType: string }
type ToolResult = {
  content: TextOrImage[]
  isError?: boolean
  [x: string]: unknown
...
async function forward(client: DaemonClient, toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
  try {
...
const demoScriptBeatSoundShape = z.object({
  file: z.string(),
...
const demoScriptBeatShape = z.object({
  ...
  sound: demoScriptBeatSoundShape.optional(),
 ...
 */
export function createSpectraServer(client: DaemonClient): McpServer {
  ...
  server.tool(
    'spectra_connect',
    'Start a new UI automation session. Target: URL (web), app name (macOS), pid:<n> (one exact macOS process), sim:device (iOS/watchOS). If repoPath is set, the launcher first boots a dev server / macOS app from that directory and uses its resolved URL/app-name as the effective target.',
    {
    ...
    async (args) => forward(client, 'spectra_connect', args),
  )
  ...
  server.tool(
    ...
    async (args) => forward(client, 'spectra_snapshot', args),
  )
  ...
  server.tool(
    ...
    async (args) => forward(client, 'spectra_act', args),
  )
  ...
  server.tool(
    ...
    async (args) => {
      ...
      try {
        const result = (await forwardTool(client, 'spectra_step', args)) as Record<string, unknown>
        const { screenshot, ...textResult } = result
        const content: TextOrImage[] = [
  ...
  server.tool(
    ...
    async (args) => forward(client, 'spectra_capture', args),
  )
  ...
  server.tool(
    ...
    { readOnlyHint: true, destructiveHint: false, idempotentHint: true },
    async (args) => forward(client, 'spectra_analyze', args),
  )
  ...
  server.tool(
    ...
    { readOnlyHint: false, destructiveHint: false, idempotentHint: false },
    async (args) => forward(client, 'spectra_discover', args),
  )
  ...
  server.tool(
    ...
    { readOnlyHint: false, destructiveHint: true, idempotentHint: false },
    async (args) => forward(client, 'spectra_walkthrough', args),
  )

  server.tool(
    'spectra_llm_step',
    'Execute a fully-formed action plan from a client-side LLM planner (the Spectra menu-bar app holds the API key; the daemon never sees it). Each action is { type, elementId, value?, intent?, waitAfterMs? }. Short-circuits on first failure unless continueOnError=true.',
    {
      sessionId: z.string().describe('Active session ID'),
    ...
    { readOnlyHint: false, destructiveHint: true, idempotentHint: false },
    async (args) => forward(client, 'spectra_llm_step', args),
  )
  ...
  server.tool(
    ...
    { readOnlyHint: false, destructiveHint: true, idempotentHint: false },
    async (args) => forward(client, 'spectra_session', args),
  )
  ...
  server.tool(
    ...
    { readOnlyHint: false, destructiveHint: false, idempotentHint: false },
    async (args) => forward(client, 'spectra_record', args),
  )
  ...
  server.tool(
    ...
    { readOnlyHint: true, destructiveHint: false, idempotentHint: true },
    async (args) => forward(client, 'spectra_replay', args),
  )
  ...
  server.tool(
    ...
    { readOnlyHint: false, destructiveHint: true, idempotentHint: false },
    async (args) => forward(client, 'spectra_library', args),
  )

  server.tool(
    'spectra_demo',
    'Produce polished agent demo video clips from screen recordings. Use action=scan to find active stretches via scene-change detection, action=polish to apply spotlight focus, captions, and speed to a set of segments and merge them, action=auto-ramp to automatically speed dead-air spans while keeping motion at real cadence, action=record-composite to drive the WINDOW-ISOLATED composite recorder (two app windows side-by-side via ScreenCaptureKit, caffeinate-wrapped so the display never sleeps, with a post-capture black-frame guard), or action=polish-clip / action=polish-script to render the rich pipeline (zoom, window chrome, caption banner + numbered step chips, optional dark-crush spotlight) from a click track or a structured beat script. Audio is preserved when the source has an audio track; stripped otherwise.',
  ...
  server.prompt(
    ...
    ({ url, steps }) => ({
      messages: [{
        role: 'user' as const,
        content: { type: 'text' as const, text: `Connect to ${url} using spectra_connect, then use spectra_walkthrough to execute these steps in order: ${steps}. Capture a screenshot after each step.` },
      }],
    }),
  )
  ...
  server.prompt(
    ...
    ({ url, feature }) => ({
      messages: [{
        role: 'user' as const,
        content: { type: 'text' as const, text: `Connect to ${url} using spectra_connect. Use spectra_analyze to find regions of interest. Navigate to the ${feature} and capture it using spectra_capture with mode=auto. Also capture with mode=region for each detected region. Save all captures.` },
      }],
    }),
  )
  ...
  server.prompt(
    ...
    ({ url }) => ({
      messages: [{
        role: 'user' as const,
        content: { type: 'text' as const, text: `Connect to ${url} using spectra_connect, then use spectra_discover to auto-navigate the entire app. Set maxDepth=3 and captureStates=true to capture loading, error, and empty states as well. Report the manifest path and a summary of what was found.` },
      }],
    }),
  )
