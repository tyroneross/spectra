...
//
// Caller-agnostic daemon contract. These method shapes describe what the core
// daemon implements and what every adapter calls after it has handled its own
// transport/auth mechanics.
...
export type LibraryParams =
  | LibraryAddParams
  | LibraryFindParams
  | LibraryGalleryParams
  | LibraryGetParams
  | LibraryTagParams
  | LibraryDeleteParams
  | LibraryStatusParams
  | LibraryExportParams
  | LibraryMigrateFromShowcaseParams
...
export type LibraryResult =
  | { added: string; path: string; entry: LibraryCaptureEntry }
  | { count: number; captures: LibraryCaptureSummary[] }
  | LibraryGalleryResult
  | { found: false; id: string }
  | { found: true; entry: LibraryCaptureEntry }
  | { updated: false; id: string }
  | { updated: true; entry: LibraryCaptureEntry }
  | { removed: false; id: string }
...
export interface DemoScanResult {
  perMinute: Array<{ minute: number; changes: number }>
  activeRanges: Array<{ startSec: number; endSec: number }>
}
