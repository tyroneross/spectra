...
} satisfies Record<CoreApiOperation, z.ZodTypeAny>
...
export const apiOperations = Object.keys(operationCapabilities).sort() as CoreApiOperation[]
...
export const clientSurfaces = [
  ...
  'test',
  'unknown',
] as const satisfies readonly ClientSurface[]
...
export const daemonEventTypes = [
  ...
  'library.changed',
  'error',
] as const satisfies readonly DaemonEventType[]
...
export const apiRequestEnvelopeSchema = z.object({
  ...
  requestId: z.string(),
  operation: z.enum(apiOperations as [CoreApiOperation, ...CoreApiOperation[]]),
...
// ─── Frozen contract surface descriptor ────────────────────────
//
// The shape the snapshot freezes + the drift test compares. Derived entirely
// from runtime values so the snapshot reflects the live contract.
...
// ─── Compile-time exhaustiveness guards (cross-file freeze) ─────
//
// If codex adds/removes a value from any of these wire.ts type unions, the
// corresponding line below fails to typecheck until the runtime mirror (and the
// frozen snapshot) is updated. This is what makes the freeze bite across the
// FE/BE boundary.

type AssertExhaustive<Expected extends string, Actual extends Expected> =
  [Expected] extends [Actual] ? true : { __missing: Exclude<Expected, Actual> }

const _surfacesExhaustive: AssertExhaustive<ClientSurface, (typeof clientSurfaces)[number]> = true
const _capabilitiesExhaustive: AssertExhaustive<Capability, (typeof capabilities)[number]> = true
const _errorCodesExhaustive: AssertExhaustive<ApiErrorCode, (typeof apiErrorCodes)[number]> = true
const _eventTypesExhaustive: AssertExhaustive<DaemonEventType, (typeof daemonEventTypes)[number]> = true
...
const _operationsExhaustive: AssertExhaustive<CoreApiOperation, (typeof apiOperations)[number]> = true
