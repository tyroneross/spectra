...
function defaultRunBinary(binaryPath: string, args: string[]): BinaryRunResult {
...
export async function resolveFocalRect(opts: ResolveFocalRectOptions): Promise<FocalRect | undefined> {
  ...
  let parsed: WindowBoundsJson
  try {
    parsed = JSON.parse(result.stdout) as WindowBoundsJson
  } catch {
    return undefined
  }
 ...
 * Pure function.
 */
export function toFocalRect(bounds: WindowBoundsJson, canvas: CanvasSize): FocalRect | undefined {
  const { x, y, w, h } = bounds
  if (![x, y, w, h].every(Number.isFinite) || w <= 0 || h <= 0) return undefined
  if (canvas.w <= 0 || canvas.h <= 0) return undefined
...
function clampToCanvas(rect: FocalRect, canvas: CanvasSize): FocalRect {
