...
export type RecordingCaptureDefaults = Partial<Pick<
  VideoOptions,
  'fps' | 'quality' | 'hardware' | 'codec' | 'bitrate'
>>
...
export const CAPTURE_PRESETS: Record<CapturePreset, CapturePresetDefinition> = {
  docs: {
    id: 'docs',
    label: 'Docs',
...
export function getCapturePresetDefinition(
  preset?: CapturePreset,
): CapturePresetDefinition | undefined {
  return preset ? CAPTURE_PRESETS[preset] : undefined
}

export function resolveScreenshotCaptureOptions(
  input: ScreenshotPresetInput,
...
export function resolveRecordingCaptureOptions(
  input: RecordingPresetInput,
...
function compactVideoOptions(options: Partial<VideoOptions>): Partial<VideoOptions> {
  const compact: Partial<VideoOptions> = {}
  for (const [key, value] of Object.entries(options)) {
    if (value !== undefined) {
      compact[key as keyof VideoOptions] = value as never
    }
  }
  return compact
}
