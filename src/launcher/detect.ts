...
import { LauncherError, type DetectionResult } from './types.js'

export function detectRepoKind(repoPath: string): DetectionResult {
  if (!existsSync(repoPath)) {
    throw new LauncherError(
      `Repo path does not exist: ${repoPath}`,
      'Provide an absolute path to a repository directory.'
    )
  }
  ...
  if (!s.isDirectory()) {
    throw new LauncherError(
      ...
      'Provide a directory, not a file.'
    )
  }
  ...
  throw new LauncherError(
    `Could not detect a launchable surface at ${repoPath}`,
    'Expected: package.json with next/vite, a .xcodeproj/.xcworkspace, or an index.html at the root.'
  )
}
