...
export async function parseCast(filePath: string): Promise<CastFile> {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      tuple = JSON.parse(line) as [number, string, string]
    } catch {
...
export function searchCast(cast: CastFile, pattern: string | RegExp): CastEvent[] {
  const regex = typeof pattern === 'string' ? new RegExp(pattern) : pattern
  return cast.events.filter(e => regex.test(e.data))
}

export function extractCommands(cast: CastFile): string[] {
  return cast.events
...
export function formatCastSummary(cast: CastFile): string {
